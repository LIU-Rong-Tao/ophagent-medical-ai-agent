...
v0.6.6 完整实验脚本、规则实现、逐 backbone 结果和评估表格已合并回明线。当前结果仍属于研究验证，不作为正式临床评估结论；后续如进入正式协作，将根据实际模型输出接口和数据格式进行适配交付。
...
v0.6.5 保留为展示入口；v0.6.6 已合并为当前主要技术验证结果，完整预审排序、强基线对比与选择性复核产物见 `experiments/summary/v0_6_6/`。
